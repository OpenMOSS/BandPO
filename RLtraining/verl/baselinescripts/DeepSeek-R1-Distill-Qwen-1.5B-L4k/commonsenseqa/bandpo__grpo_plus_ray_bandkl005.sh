...
: "${BandPODir_LargeData:?BandPODir_LargeData is not set. Ensure the environment is initialized by running: source /path_to_BandPO/init.sh}"
...
csqa_train=$BandPODir/data/dataset/commonsenseqa/train_4k.parquet
csqa_val=$BandPODir/data/dataset/commonsenseqa/arc_challenge_val.parquet

# to modify
project_name="VERL_commonsenseqa_DeepSeekR1DistillQwen1.5B"
exp_name="bandpo__grpo_plus_ray_bandkl005"
model_path="$BandPODir/data/models/deepseek-ai/DeepSeek-R1-Distill-Qwen-1.5B"

max_prompt_length=$((1024 * 1))
max_response_length_real=$((1024 * 4))
...
# If URL is empty, consider the filter disabled; notify user and skip health check.
if [[ -z "$rambling_filter_url" ]]; then
  echo "[INFO] Rambling_filter_url is empty: Online rambling filter disabled."
else
  # Perform health check only if URL is set; exit if the check fails.
...
ray job submit --no-wait --runtime-env="${RUNTIME_ENV}" \
    ...
    data.train_files="['$csqa_train']" \
    data.val_files="['$csqa_val']" \
    ...
    +actor_rollout_ref.actor.band_numerical_solver="bisect" \
    +actor_rollout_ref.actor.band_use_log_domain=False \
