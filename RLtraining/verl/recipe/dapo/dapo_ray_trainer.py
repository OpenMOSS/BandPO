...
class RayDAPOTrainer(RayPPOTrainer):
    ...
    def _run_online_rambling_filter(
        ...
        *,
        ...
        action = getattr(self, "_rambling_filter_action", "modify_reward")

        if action == "modify_reward":
            # 原有逻辑：中性化 reward，不删样本
            for i in bad_idx:
            ...
            # 新逻辑：不改 reward，本 step 先标记出“需要整组删除的 prompt uid”
            bad_prompt_uids = set()
            for i in bad_idx:
                extras = per_sample_extras[i]
                if not isinstance(extras, dict):
                ...
                uid_i = uids[i] if uids is not None else None
                if uid_i is not None:
                    bad_prompt_uids.add(str(uid_i))

            if bad_prompt_uids:
                # 注意：这里不要写入 non_tensor_batch，否则 DataProto 在 select_idxs 时会按样本索引，
                # 而这个列表的长度是“坏 prompt 个数”，不是 B，容易越界。
                prev = list(
                    map(
                        str,
                        new_batch.meta_info.get("rambling_drop_prompt_uids", []),
                    )
                )
                merged = list(dict.fromkeys(prev + list(bad_prompt_uids)))  # 保序去重
                # 存在 meta_info 里，只用于本 step 的 group filter 逻辑
                new_batch.meta_info["rambling_drop_prompt_uids"] = merged
            # if bad_prompt_uids:
            #     prev = list(
            #         map(
            #             str,
            #             new_batch.non_tensor_batch.get("rambling_drop_prompt_uids", []),
            #         )
            #     )
            #     merged = list(dict.fromkeys(prev + list(bad_prompt_uids)))  # 保序去重
            #     new_batch.non_tensor_batch["rambling_drop_prompt_uids"] = np.array(
            #         merged, dtype=object
            #     )
        else:
            # 理论上不会走到这里，因为 fit() 里已经做过合法性检查
            msg = (
                f"[WARN] Unknown rambling_filter_action={action}, "
                f"fallback to modify_reward semantics."
            )
            raise RuntimeError(msg)
    ...
    def fit(self):
        ...
        # 新增：rambling filter 的处理方式：modify_reward / delete_group
        self._rambling_filter_action = str(
            self.config.trainer.get("rambling_filter_action", "modify_reward")
        ).lower()
        if self._rambling_filter_action not in ("modify_reward", "delete_group"):
            print(
                f"[WARN] Unknown trainer.rambling_filter_action={self._rambling_filter_action}, "
                f"fallback to 'modify_reward'"
            )
            self._rambling_filter_action = "modify_reward"
        if rambling_filter_url:
            ...
            print(
                ...
                f"action={self._rambling_filter_action}, "
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    # === 统一的 group 过滤逻辑：dynamic sampling + rambling delete_group ===
                    use_ds_original = bool(self.config.algorithm.filter_groups.enable)
                    use_rambling_delete = (
                        self._rambling_filter_client is not None
                        and getattr(self, "_rambling_filter_action", "modify_reward") == "delete_group"
                    )
                    use_group_filter = use_ds_original or use_rambling_delete
                    print(f"use_ds_original: {use_ds_original}, use_rambling_delete: {use_rambling_delete}, use_group_filter: {use_group_filter}")
                    if not use_group_filter:
                        # 不开启 dynamic sampling，且 rambling_action 不是 delete_group：
                        # 直接使用当前 new_batch 作为训练 batch 的一部分
                        batch = new_batch
                    else:
                        ...
                        # 所有 trajectory 的 prompt uid（已在前面 repeat 过），统一转成 str 方便集合运算
                        uids_arr = list(
                            map(str, candidate_batch_before_filter.non_tensor_batch["uid"])
                        )
                        # prompt 级 uid：按出现顺序保序去重
                        all_prompt_uids = list(dict.fromkeys(uids_arr))

                        # ---------- 1) 原始 dynamic sampling 的 bad uids（按 metric 标准差） ----------
                        ds_bad_uids: set[str] = set()
                        if use_ds_original:
                            metric_name = self.config.algorithm.filter_groups.metric
                            if metric_name == "seq_final_reward":
                                # Turn to numpy for easier filtering
                                candidate_batch_before_filter.non_tensor_batch["seq_final_reward"] = (
                                    candidate_batch_before_filter.batch["token_level_rewards"]
                                    .sum(dim=-1)
                                    .numpy()
                                )
                            elif metric_name == "seq_reward":
                                candidate_batch_before_filter.non_tensor_batch["seq_reward"] = (
                                    candidate_batch_before_filter.batch["token_level_scores"]
                                    .sum(dim=-1)
                                    .numpy()
                                )
                            ...
                            for uid, metric_val in zip(
                                uids_arr,
                                candidate_batch_before_filter.non_tensor_batch[metric_name],
                                strict=True,
                            ):
                            ...
                            ds_bad_uids = set(all_prompt_uids) - set(kept_prompt_uids_ds)

                        # ---------- 2) rambling delete_group 的 bad uids ----------
                        rambling_bad_uids: set[str] = set()
                        if use_rambling_delete:
                            # 从 meta_info 读取；这是前面 _run_online_rambling_filter 写进去的
                            rambling_drop_prompt_uids = candidate_batch_before_filter.meta_info.get(
                                "rambling_drop_prompt_uids", []
                            )
                            # 统一转成 str
                            rambling_bad_uids = set(map(str, rambling_drop_prompt_uids))

                        # # ---------- 2) rambling delete_group 的 bad uids ----------
                        # rambling_bad_uids: set[str] = set()
                        # if use_rambling_delete:
                        #     rambling_drop_prompt_uids = candidate_batch_before_filter.non_tensor_batch.get(
                        #         "rambling_drop_prompt_uids", []
                        #     )
                        #     # 统一转成 str
                        #     rambling_bad_uids = set(map(str, rambling_drop_prompt_uids))
                        ...
                        # ---------- 3) 合并两方的 bad uids ----------
                        combined_bad_uids = ds_bad_uids | rambling_bad_uids
                        if combined_bad_uids:
                            kept_prompt_uids = [
                                uid for uid in all_prompt_uids if uid not in combined_bad_uids
                            ]
                        else:
                            kept_prompt_uids = all_prompt_uids

                        num_prompt_in_batch += len(kept_prompt_uids)

                        # ---------- 4) 计算 kept / dropped 的 trajectory 索引 ----------
                        kept_traj_idxs: list[int] = []
                        for idx, traj_uid in enumerate(uids_arr):
                            if traj_uid in kept_prompt_uids:
                                kept_traj_idxs.append(idx)

                        all_idxs = list(range(len(uids_arr)))
                        kept_set = set(kept_traj_idxs)
                        dropped_traj_idxs = [i for i in all_idxs if i not in kept_set]

                        if getattr(self, "_rambling_filter_debug", False) and use_group_filter:
                            # 1) 所有 bad uid 的所有样本必须在 dropped_traj_idxs 里
                            for idx, uid in enumerate(uids_arr):
                                if uid in combined_bad_uids:
                                    assert idx in dropped_traj_idxs, (
                                        f"[DEBUG] uid {uid} is in combined_bad_uids, "
                                        f"but its traj idx {idx} is NOT dropped."
                                    )
                                else:
                                    assert idx in kept_traj_idxs, (
                                        f"[DEBUG] uid {uid} is NOT in combined_bad_uids, "
                                        f"but its traj idx {idx} IS dropped."
                                    )
                            print("[DEBUG] group filter summary at step", self.global_steps)
                            print("  ds_bad_uids       :", sorted(list(ds_bad_uids)))
                            print("  rambling_bad_uids :", sorted(list(rambling_bad_uids)))
                            print("  combined_bad_uids :", sorted(list(combined_bad_uids)))
                            print("  kept_prompt_uids  :", kept_prompt_uids)

                            # 统计每个 uid 实际保留/丢弃多少条 sample
                            uid2kept = defaultdict(int)
                            uid2drop = defaultdict(int)
                            for idx, uid in enumerate(uids_arr):
                                if idx in kept_traj_idxs:
                                    uid2kept[uid] += 1
                                else:
                                    uid2drop[uid] += 1
                            for uid in all_prompt_uids:
                                print(
                                    f"    uid={uid}: kept={uid2kept[uid]}, dropped={uid2drop[uid]}"
                                )

                        # === 收集被过滤掉的子批，用于 JSONL 记录 ===
                        ...
                        # === 保留的 traj 拼回 batch ===
                        new_batch = candidate_batch_before_filter[kept_traj_idxs]
                        batch = new_batch if batch is None else DataProto.concat([batch, new_batch])

                        prompt_bsz = self.config.data.train_batch_size
                        if num_prompt_in_batch < prompt_bsz:
                            print(f"{num_prompt_in_batch=} < {prompt_bsz=}")
                            # 即使 dynamic sampling 关闭（enable=False），也复用它的 max_num_gen_batches 配置
                            max_num_gen_batches = self.config.algorithm.filter_groups.get(
