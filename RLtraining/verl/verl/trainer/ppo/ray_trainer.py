...
class RayPPOTrainer:
    ...
    def _validate(self):
        ...
        # NEW: collect response length (token-level) over all validation outputs
        sample_resp_token_lens = []
        ...
        for test_data in self.val_dataloader:
            ...
            # NEW: compute per-sample response length in tokens (exclude paddings)
            with torch.no_grad():
                # output_ids: Tensor[B, L_resp]
                outputs_tensor = output_ids
                pad_id = self.tokenizer.pad_token_id
                if pad_id is not None:
                    non_pad = outputs_tensor.ne(pad_id)
                else:
                    # Fallback：如果 tokenizer 没设 pad_id，就把 !=0 当作有效 token
                    non_pad = outputs_tensor.ne(0)
                resp_lens = non_pad.sum(dim=-1).cpu().tolist()  # List[int], 当前 batch 的长度为 B
            sample_resp_token_lens.extend(resp_lens)
        ...
        # === NEW: 把本次 validate 的所有样本写入 validate_record_path（如已配置） ===
        try:
            self._write_validate_records(
                inputs=sample_inputs,
                outputs=sample_outputs,
                gts=sample_gts,
                scores=sample_scores,
                ...
                uids=np.array(sample_uids, dtype=object),
                resp_token_lens=sample_resp_token_lens,
            )
        except Exception as e:
            # 不让写文件影响训练，只打一条 warning
            print(
                f"[WARN] _write_validate_records failed during validation at global_step={self.global_steps}: {e}",
                flush=True,
            )
        ...
        # NEW: response_len_tokens stats, per data_source + overall, all in val-core
        if len(sample_resp_token_lens) > 0:
            resp_len_arr = np.array(sample_resp_token_lens, dtype=np.float32)
            assert len(resp_len_arr) == len(data_sources), (
                f"response length size mismatch: {len(resp_len_arr)=}, {len(data_sources)=}"
            )

            # 1) overall stats across all validation samples
            metric_dict["val-core/all/response_len_tokens/min"] = float(resp_len_arr.min())
            metric_dict["val-core/all/response_len_tokens/max"] = float(resp_len_arr.max())
            metric_dict["val-core/all/response_len_tokens/mean"] = float(resp_len_arr.mean())

            # 2) per data_source stats
            # data_sources 可能是 dtype=object，这里用 np.unique 做分组
            for ds in np.unique(data_sources):
                mask = (data_sources == ds)
                ds_lens = resp_len_arr[mask]
                # 避免空切片（理论上不会，防御性写法）
                if ds_lens.size == 0:
                    continue
                ds_str = str(ds)
                prefix = f"val-core/{ds_str}/response_len_tokens"
                metric_dict[f"{prefix}/min"] = float(ds_lens.min())
                metric_dict[f"{prefix}/max"] = float(ds_lens.max())
                metric_dict[f"{prefix}/mean"] = float(ds_lens.mean())
    ...
    # ====== Rollout record helpers (driver-side JSONL, single-writer) ======  # NEW
    ...
    def _to_py(obj):  # NEW
        ...
        if isinstance(obj, (list, tuple)):
            return [RayPPOTrainer._to_py(x) for x in obj]
        if isinstance(obj, dict):
            return {k: RayPPOTrainer._to_py(v) for k, v in obj.items()}
        return obj

    def _rr_open_if_needed(self):  # NEW
    ...
    def _rr_flush(self, force=False):  # NEW
        if getattr(self, "_rr_fh", None) is None:
            return
    ...
    def _decode_texts(self, token_ids, skip_special_tokens=True):  # NEW
        # token_ids: Tensor[B, L]
        return self.tokenizer.batch_decode(token_ids, skip_special_tokens=skip_special_tokens)

    def _make_rr_lines_from_batch(  # NEW
        self,
        batch,
        *,
        epoch: int,
        ...
        filter_state: str = "kept",
        filter_reason: str | None = None,
    ):
        """
        Build JSONL lines (strings) for each sample in a DataProto batch.
        Requires batch has: attention_mask, response_mask
        """
        import torch
        if batch is None:
            return []
        ...
        resp_mask = batch.batch.get("response_mask", None) # [B, L]
        if resp_mask is None:
            try:
                resp_mask = compute_response_mask(batch)
                batch.batch["response_mask"] = resp_mask
            except Exception:
                # 兜底：没有 response_mask 时，用全 1，不影响 JSON 落盘
                resp_mask = torch.ones_like(attn)
        ...
        if (old_log_probs is not None) and (ref_log_prob is not None):
            kl_token_log_ratio = (old_log_probs - ref_log_prob)
        kl_mean, kl_sum = masked_mean_sum(kl_token_log_ratio)

        # token 级 entropy（如果 compute_log_prob 阶段保留了）
        ...
        for i in range(B):
            rec = {
                ...
                "filter_state": filter_state,         # 非 DAPO 场景恒为 kept
    ...
    def _write_rollout_records(self, *, kept_batch, epoch: int, global_step: int, actor_output_metrics: dict | None = None):  # NEW
        if not getattr(self, "_rr_path", ""):
            return
        self._rr_open_if_needed()
        lines = self._make_rr_lines_from_batch(
            kept_batch,
            epoch=epoch,
        ...
        self._rr_buf.extend(lines)
        self._rr_flush(force=False)

    # ====== Validation record helpers (driver-side JSONL, single-writer) ======  # NEW

    def _vr_open_if_needed(self):  # NEW
        """Open validation JSONL file lazily if trainer.validate_record_path is set."""
        if not getattr(self, "_vr_path", ""):
            return
        if getattr(self, "_vr_fh", None) is None:
            dirpath = os.path.dirname(self._vr_path) or "."
            os.makedirs(dirpath, exist_ok=True)
            # 以 append 模式打开，不截断已有内容；如果文件不存在会自动创建
            self._vr_fh = open(self._vr_path, "a", encoding="utf-8")
            self._vr_buf = []

    def _vr_flush(self, force: bool = False):  # NEW
        if getattr(self, "_vr_fh", None) is None:
            return
        if self._vr_buf and (force or len(self._vr_buf) >= 64):
            self._vr_fh.write("".join(self._vr_buf))
            self._vr_fh.flush()
            self._vr_buf.clear()

    def _vr_close(self):  # NEW
        if getattr(self, "_vr_fh", None) is not None:
            self._vr_flush(force=True)
            try:
                self._vr_fh.close()
            finally:
                self._vr_fh = None

    def _make_validate_record_lines(  # NEW
        self,
        *,
        inputs,
        outputs,
        ...
        """Build JSONL lines for validation samples."""
        n = len(inputs)
        if n == 0:
            return []
        ...
        assert (
            ...
            and len(scores) == n
            and len(uids) == n
            and len(resp_token_lens) == n
            and len(data_sources) == n
        ), (
            f"validate record size mismatch: "
            f"{n=}, {len(outputs)=}, {len(gts)=}, {len(scores)=}, "
            f"{len(uids)=}, {len(resp_token_lens)=}, {len(data_sources)=}"
        )

        # validation 时的 n（一个 prompt 采样多少个 response）
        try:
        ...
        for i in range(n):
            ...
            rec = {
                ...
                # 一组 n 个 sample 共享的 uid（prompt 粒度）
                "uid": None if uids is None else str(uids[i]),
                # 分组信息
                "prompt_group_index": int(group_idx),
                "repeat_index": int(repeat_idx),
                "val_n": int(val_n),
                # 文本内容
                "prompt_text": inputs[i],
                "response_text": outputs[i],
                "ground_truth": gts[i],
                # reward_fn 的总分
                "score": float(scores[i]) if scores is not None else None,
                # 响应 token 长度（便于之后做长度分析）
                "response_len_tokens": int(resp_token_lens[i]),
            }

            # 把 reward_extra_info 里和每个样本对齐的字段也写进去
            for k, v in reward_extra_infos_dict.items():
                if len(v) == 0:
                    continue
                if len(v) != n:
                    # 理论上在 _validate 里已经做过长度校验，这里再防御一次
                    continue
                rec[k] = self._to_py(v[i])

            lines.append(json.dumps(rec, ensure_ascii=False) + "\n")

        return lines

    def _write_validate_records(  # NEW
        self,
        *,
        inputs,
        outputs,
        ...
        """Append validation sample records into trainer.validate_record_path (if configured)."""
        if not getattr(self, "_vr_path", ""):
            return
        self._vr_open_if_needed()
        lines = self._make_validate_record_lines(
            inputs=inputs,
            outputs=outputs,
            gts=gts,
            scores=scores,
            ...
            data_sources=data_sources,
            uids=uids,
            resp_token_lens=resp_token_lens,
        )
        if not lines:
            return
        self._vr_buf.extend(lines)
        self._vr_flush(force=False)
    ...
    def fit(self):
        ...
        # === validation record config ===  # NEW
        self._vr_path = str(self.config.trainer.get("validate_record_path", "") or "")
        self._vr_fh = None
        self._vr_buf = []
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                actor_output_metrics = None  # NEW: 供后续 _write_rollout_records 使用
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("old_log_prob", timing_raw, color="blue"):
                        ...
                        entropys_tok = old_log_prob.batch["entropys"].detach()      # NEW: token-level entropy 保留到 batch
                        ...
                        batch.batch["entropys_tok"] = entropys_tok                    # NEW
                    ...
                    # === [NEW] 统一写当前 step 的 kept 样本 JSONL 记录 ===
                    try:
                        # 兜底，保证有 response_mask
                        ...
                        self._write_rollout_records(
                            ...
                            actor_output_metrics=actor_output_metrics,  # 可能为 None，helper 已兼容
                        )
                    except Exception as e:
