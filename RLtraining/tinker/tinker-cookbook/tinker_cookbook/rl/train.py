...
@scope
def check_stream_dataset_batches(dataset: RLDataset, expected_groups_per_batch: int) -> None:
    """
    严格用训练时同样的接口：dataset.get_batch(i_batch)
    只检查每个 batch 吐出来的 EnvGroupBuilder 数量是否满足 stream 要求。
    """
    n_batches = len(dataset)
    logger.info(
        f"[dataset_check] Start: num_batches={n_batches}, expected_groups_per_batch={expected_groups_per_batch}"
    )
    for i_batch in range(n_batches):
        builders = dataset.get_batch(i_batch)
        if len(builders) != expected_groups_per_batch:
            raise ValueError(
                f"[dataset_check] FAILED at i_batch={i_batch}: "
                f"len(builders)={len(builders)} != expected={expected_groups_per_batch}"
            )
        if (i_batch + 1) % 50 == 0:
            logger.info(f"[dataset_check] progress {i_batch+1}/{n_batches}")
    logger.info("[dataset_check] ✅ PASSED: all batches match expected_groups_per_batch")
...
class Config:
    ...
    check_dataset_before_train: bool = False  # ✅ 新增：训练前先跑一遍检查
    check_dataset_only: bool = False          # ✅ 新增：只检查不训练，检查通过就退出
...
def _add_validate_core_all_metrics(
    metrics: dict[str, Any],
    evaluators: list[SamplingClientEvaluator],
) -> dict[str, Any]:
    """
    Aggregate validate_core/all/* using additive validate_core/<name>/_raw/* stats
    emitted by RLTestSetEvaluator.

    - pass@weighted uses num_problems weighting: passed_problems / num_problems
    - mean@weighted and avg_*_tokens use num_samples weighting: sum / num_samples
    """
    sum_num_problems = 0.0
    sum_num_samples = 0.0
    sum_passed_problems = 0.0
    sum_correct_samples = 0.0
    sum_prompt_tokens = 0.0
    sum_response_tokens = 0.0
    sum_total_tokens = 0.0

    for evaluator in evaluators:
        ev_name = _get_evaluator_name(evaluator)
        if not ev_name:
            continue  # unnamed evaluators won't participate

        raw_prefix = f"validate_core/{ev_name}/_raw/"
        if f"{raw_prefix}num_problems" not in metrics:
            # backward-compat / non-RLTestSetEvaluator
            continue

        sum_num_problems += float(metrics.get(f"{raw_prefix}num_problems", 0.0))
        sum_num_samples += float(metrics.get(f"{raw_prefix}num_samples", 0.0))
        sum_passed_problems += float(metrics.get(f"{raw_prefix}passed_problems", 0.0))
        sum_correct_samples += float(metrics.get(f"{raw_prefix}correct_samples", 0.0))
        sum_prompt_tokens += float(metrics.get(f"{raw_prefix}sum_prompt_tokens", 0.0))
        sum_response_tokens += float(metrics.get(f"{raw_prefix}sum_response_tokens", 0.0))
        sum_total_tokens += float(metrics.get(f"{raw_prefix}sum_total_tokens", 0.0))

    if sum_num_problems > 0 and sum_num_samples > 0:
        metrics["validate_core/all/pass@weighted"] = sum_passed_problems / sum_num_problems
        metrics["validate_core/all/mean@weighted"] = sum_correct_samples / sum_num_samples
        metrics["validate_core/all/avg_prompt_tokens"] = sum_prompt_tokens / sum_num_samples
        metrics["validate_core/all/avg_response_tokens"] = sum_response_tokens / sum_num_samples
        metrics["validate_core/all/avg_total_tokens"] = sum_total_tokens / sum_num_samples

        metrics["validate_core/all/_raw/num_problems"] = sum_num_problems
        metrics["validate_core/all/_raw/num_samples"] = sum_num_samples
        metrics["validate_core/all/_raw/passed_problems"] = sum_passed_problems
        metrics["validate_core/all/_raw/correct_samples"] = sum_correct_samples
        metrics["validate_core/all/_raw/sum_prompt_tokens"] = sum_prompt_tokens
        metrics["validate_core/all/_raw/sum_response_tokens"] = sum_response_tokens
        metrics["validate_core/all/_raw/sum_total_tokens"] = sum_total_tokens
        metrics["validate_core/all/_raw/avg_k"] = sum_num_samples / sum_num_problems
    else:
        # still emit keys to make dashboards stable
        metrics["validate_core/all/pass@weighted"] = np.nan
        metrics["validate_core/all/mean@weighted"] = np.nan
        metrics["validate_core/all/avg_prompt_tokens"] = np.nan
        metrics["validate_core/all/avg_response_tokens"] = np.nan
        metrics["validate_core/all/avg_total_tokens"] = np.nan

    return metrics



def _prune_validate_core_raw_metrics(metrics: dict[str, Any]) -> None:
    """
    Remove intermediate validate_core/*/_raw/* metrics to keep logs compact.
    This should be called AFTER validate_core/all aggregation is computed.
    """
    keys_to_delete = []

    for k in list(metrics.keys()):
        # 1) 原本就删掉所有 _raw
        if k.startswith("validate_core/") and "/_raw/" in k:
            keys_to_delete.append(k)
            continue

        # 2) ✅ 额外删掉 prompt/total 的“平均 token”指标（想恢复就注释掉这一段）
        if k.startswith("validate_core/") and (
            k.endswith("/avg_prompt_tokens") or k.endswith("/avg_total_tokens")
        ):
            keys_to_delete.append(k)
...
async def run_evaluations_parallel(
    ...
    # Add compact aggregated validate_core/all metrics
    _add_validate_core_all_metrics(metrics, evaluators)

    # prune intermediate raw stats to reduce log clutter
    _prune_validate_core_raw_metrics(metrics)
...
async def main(
    ...
    # ✅ 先构建 dataset（因为 check 只依赖 dataset.get_batch 接口）
    dataset, maybe_test_dataset = await cfg.dataset_builder()
    num_batches = len(dataset)
    logger.info(f"Dataset built: num_batches={num_batches}")
    # ✅ 如果启用了 stream，并且要求 check：在创建 tinker client 之前就 check（fail fast）
    need_check = (
        cfg.stream_minibatch_config is not None
        and (cfg.check_dataset_before_train or cfg.check_dataset_only)
    )
    if need_check:
        expected = cfg.stream_minibatch_config.groups_per_batch
        check_stream_dataset_batches(dataset, expected_groups_per_batch=expected)
        if cfg.check_dataset_only:
            ml_logger.close()
            logger.info("check_dataset_only=True, exiting without training.")
            return
    ...
    # # ✅ Dataset streaming check (same interface as training loop)
    # if cfg.stream_minibatch_config is not None and (cfg.check_dataset_before_train or cfg.check_dataset_only):
    #     expected = cfg.stream_minibatch_config.groups_per_batch
    #     check_stream_dataset_batches(dataset, expected_groups_per_batch=expected)
    #     if cfg.check_dataset_only:
    #         ml_logger.close()
