...
输入 -> 输出字段映射与规则：
- 输入 Parquet：任意 schema
- 必需键：
    --question_key：问题列名
    --answer_key：答案列名（最终答案）
- 可选键：
    --solution_key：解题过程列名；未提供或该行为空/NaN/空串 -> extra_info.solution = 答案
    --keep_id + --id_key：是否保留 id 到 extra_info.index
- 其它：
    --data_source：顶层 data_source 字段
    --data_source_extra_info：写入 extra_info["data_source"]（字符串）
    --ability：默认 "MATH"
    --reward_style：默认 "rule-lighteval/MATH_v2"
    --split：默认 "train"，可选 "train"/"test"
...
输出 Parquet 列：
    ...
    extra_info: struct<
        ...
        raw_problem: string,
        solution: string,
        data_source: string   # 来自 --data_source_extra_info
    >
    split: string ("train" or "test")

写出规则：
- 与源文件同目录，文件名在原始名后追加 "_verl"（如 a.parquet -> a_verl.parquet）
- 压缩：zstd
用法（CLI）：
...
def convert_parquet_to_verl(
    ...
    # Define Arrow schema with nested types
