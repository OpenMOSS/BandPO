...
def aggregate_per_step(values: np.ndarray, step_idx: Dict[int, np.ndarray], agg: str) -> Tuple[List[int], List[float]]:
    xs, ys = [], []
...
def scatter_one_metric(
    ...
    # scatter by class
    for cls_val, marker, edgec, label in [
        (0, "^", COLOR_WRONG,   "wrong"),
        (1, "o", COLOR_CORRECT, "correct"),
        (2, "s", COLOR_UNKNOWN, "no_judge"),
    ]:
        mask = (cls_id == cls_val) & np.isfinite(values)
        if not np.any(mask):
            continue
    ...
    # per-step aggregation line
    agg_x, agg_y = aggregate_per_step(values, step_idx, agg=agg)
    if agg_x:
    ...
    ax.set_title(title)
    ax.legend(loc="lower right", ncol=4, fontsize="small", frameon=True)
    fig.tight_layout()
