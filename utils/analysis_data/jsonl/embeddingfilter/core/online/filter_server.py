...
class ServerConfig:
    ...
    # ✅ 新增：是否把子集过滤结果映射回完整 JSONL 行序
    subset_mapping: bool = False
...
class FilterService:
    def __init__(self, cfg: ServerConfig):
        ...
        if self.debug_out_dir is not None:
            ...
            if not cfg.quiet:
                print(f"[Service] Debug JSONL root dir = {self.debug_out_dir}")
    ...
    def run_filter_on_jsonl(
        ...
        project_name : Optional[str]
            （可选）本次调用所属的 project 名，用于 debug 分目录。
        exp_name : Optional[str]
            （可选）本次调用所属的 experiment 名，用于 debug 分目录。

        注意：
        - scan_offsets 阶段使用的是 ServerConfig 中的 kept_or_dropped + scan_cls，
          这一步就已经完成了一次基于 kept / cls 的“子集选择”；
        ...
        """
        t_start = time.time()
        ...
        if not self.cfg.quiet:
            print(
                f"[Service] === Filter request on {path} (debug={self.debug}) ===",
                flush=True,
            )
        ...
        # 如果 scan 根本没有选出任何行（比如全是 dropped），直接返回“全部保留”
        if scan_stats["num_selected"] == 0:
            total_lines = int(scan_stats["num_total"])
            return (
                np.ones(total_lines, dtype=bool),
                {
                    "num_total": total_lines,
                    "num_selected": 0,
                    "num_bad": 0,
                    "num_good": total_lines,
                },
                {},
            )
        t_end_stage0 = time.time()
        t_stage0 = t_end_stage0 - t_start
        if not self.cfg.quiet:
            print(f"[Timing] Stage0 scan_offsets: {t_stage0:.1f}s", flush=True)
        ...
        # ⚠️ 在 FastAPI/Uvicorn 这类服务进程里严禁再用 fork，
        # 否则容易和已有线程/子进程死锁，所以这里强制使用 spawn。
        ctx = mp.get_context("spawn")
        ...
        t_end_stage1 = time.time()
        t_stage1 = t_end_stage1 - t_end_stage0
        if not self.cfg.quiet:
            print(f"[Timing] Stage1 CPU metrics + segmentation: {t_stage1:.1f}s", flush=True)
        ...
        t_stage2 = t_end_stage2 - t_end_stage1
        if not self.cfg.quiet:
            print(f"[Timing] Stage2 sentence embedding: {t_stage2:.1f}s", flush=True)
        ...
        t_end_stage3 = time.time()
        t_stage3 = t_end_stage3 - t_end_stage2
        if not self.cfg.quiet:
            print(f"[Timing] Stage3 embedding-based metrics + rambling_score: {t_stage3:.1f}s", flush=True)
        # ------- Pattern filtering（只在“子集”上；不再额外按 cls 过滤） -------
        if not self.cfg.quiet:
            print("[Service][Filter] Running pattern-based filtering on subset ...", flush=True)

        # 这里不再根据 cls 再筛一次，target_mask=None 表示“对子集中的所有样本应用 pattern”
        keep_mask_subset, bad_mask_subset, pattern_hits = fa.build_filter_mask_by_patterns(
            samples_df,
            target_mask=None,
        )

        subset_total = len(samples_df)
        subset_bad = int(bad_mask_subset.sum())
        subset_good = subset_total - subset_bad

        if not self.cfg.quiet:
            print(
                f"[Service][Filter] subset_total={subset_total:,}, "
                f"subset_bad={subset_bad:,}, subset_good={subset_good:,}",
                flush=True,
            )
        t_end_stage4 = time.time()
        t_stage4 = t_end_stage4 - t_end_stage3
        if not self.cfg.quiet:
            print(f"[Timing] Stage4 pattern filtering: {t_stage4:.1f}s", flush=True)
        
        # ------- Debug JSONL 输出（可选） -------
        debug_files: Dict[str, str] = {}
        if self.debug and self.debug_jsonl_enabled and self.debug_out_dir is not None:
            # 以 CLI 的 --debug-out-dir 作为根目录；如果请求里带 project/exp，就追加两级子目录
            debug_root = self.debug_out_dir
            if project_name and exp_name:
                debug_root = debug_root / project_name / exp_name
            elif project_name or exp_name:
                # 只传了一个，给个 warning，退回根目录，避免目录结构混乱
                if not self.cfg.quiet:
                    print(
                        f"[Service][Debug] WARN: only one of project_name/exp_name provided "
                        f"(project_name={project_name!r}, exp_name={exp_name!r}), "
                        f"use root debug dir {debug_root}",
                        flush=True,
                    )
            ...
            if not self.cfg.quiet:
                ...
                if project_name and exp_name:
                    print(
                        f"[Service][Debug] project={project_name}, exp={exp_name}, dir={debug_root}",
                        flush=True,
                    )
            ...
            debug_files = self._write_debug_jsonl(
                ...
                bad_mask=bad_mask_subset,
                pattern_hits=pattern_hits,
                jsonl_path=path,
                debug_out_dir=debug_root,
            )


        # ------- 将“子集”过滤结果映射回完整 JSONL 行序（如果启用 subset_mapping） -------
        if self.cfg.subset_mapping:
            total_lines = int(scan_stats["num_total"])
            if not self.cfg.quiet:
                print(
                    f"[Service][Map] subset_mapping enabled: total_lines={total_lines}, "
                    f"subset_total={subset_total}",
                    flush=True,
                )

            # 1) 构建 offset -> line_idx 映射
            ...
            with cfg.jsonl.open("rb", buffering=1024 * 1024) as fsrc:
                ...
                while True:
                    ...
                    if not b:
                        break
                    offset_to_idx[pos] = line_idx
                    pos += len(b)
                    line_idx += 1

            if line_idx != total_lines and not self.cfg.quiet:
                print(
                    f"[Service][Map] WARN: counted {line_idx} lines from file, "
                    f"but scan_stats.num_total={total_lines}",
                    flush=True,
                )

            # 2) 初始化完整 mask：默认全部 keep=True（即未进入子集的行完全不动）
            full_keep_mask = np.ones(total_lines, dtype=bool)

            # 3) 使用 byte_offset 把子集行映射回原始行号，并应用过滤结果
            subset_offsets = samples_df["byte_offset"].astype(int).tolist()
            for i, off in enumerate(subset_offsets):
                full_idx = offset_to_idx.get(off, None)
                if full_idx is None:
                    raise RuntimeError(
                        f"[Service][Map] byte_offset={off} not found in offset_to_idx；"
                        "JSONL 可能在 scan 后被修改。"
                    )
                if not bool(keep_mask_subset[i]):
                    full_keep_mask[full_idx] = False

            num_total = total_lines
            num_bad = int((~full_keep_mask).sum())
            ...
            stats = {
                ...
                "subset_bad": subset_bad,
                "subset_good": subset_good,
            }

            final_mask = full_keep_mask
        else:
            # 兼容旧行为：只返回子集上的 keep_mask
            num_total = subset_total
            num_bad = subset_bad
            num_good = subset_good
            stats = {
                "num_total": num_total,
                "num_bad": num_bad,
                "num_good": num_good,
            }
            final_mask = keep_mask_subset

        t_end_stage5 = time.time()
        t_stage5 = t_end_stage5 - t_end_stage4
        if not self.cfg.quiet:
            print(f"[Timing] Stage5 dubug jsonl: {t_stage5:.1f}s", flush=True)
        t_end = time.time()
        if not self.cfg.quiet:
            print(f"[Service] Filter pipeline finished in {t_end - t_start:.1f}s", flush=True)

        return final_mask, stats, debug_files
    ...
    def _write_debug_jsonl(
        ...
        ----
        debug_out_dir : Path
            本次调用实际使用的 debug 目录（已经包含 project_name/exp_name 等）。
        """
        ...
        out_good = debug_out_dir / f"{base}.good.jsonl"
        out_bad = debug_out_dir / f"{base}.bad.jsonl"
...
class FilterRequest(BaseModel):
    jsonl_path: str
    # 可选：verl 侧传过来的 project / experiment 名
    project_name: Optional[str] = None
    exp_name: Optional[str] = None
    # cls 不再从 HTTP 传入，而是由 server 启动时的 --cls 决定 scan 子集
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        "--subset-mapping",
        action="store_true",
        help="开启后，scan_offsets 选出的子集过滤完会映射回完整 JSONL 行序，"
             "返回的 keep_mask 长度等于文件总行数；未在子集中的行默认 keep=True。",
    )
...
def main():
    # 统一整个进程的 multiprocessing start_method 为 spawn
    # （只能调用一次；如果已经设置过会抛 RuntimeError，这里直接忽略）
    try:
    ...
    cfg = ServerConfig(
        ...
        port=args.port,
        # ✅ 新增
        subset_mapping=bool(args.subset_mapping),
    )
