...
def run_vllm_embedding(
    ...
    # 延迟导入 vllm：只有真正选择 embed-backend='vllm' 时才会触发 CUDA 初始化
    try:
        from vllm import LLM  # type: ignore
    except ImportError as e:
        raise RuntimeError(
            "embed-backend='vllm' 需要安装 vllm 包，请先 `pip install vllm`"
        ) from e
...
def pattern_marker_loopiness_too_more(df: pd.DataFrame) -> pd.Series:
    ...
    # return df["marker_loopiness"].astype(float) > 30

def pattern_control_private_unassigned_ratio_too_large(df: pd.DataFrame) -> pd.Series:
    print("[Pattern] pattern_control_private_unassigned_ratio_too_large ...")
    if "control_private_unassigned_ratio" not in df.columns:
        return pd.Series(False, index=df.index)
    # 不能高于 0.12 -> 命中条件是 > 0.12
    return df["control_private_unassigned_ratio"].astype(float) > 0.12

def pattern_compress_ratio_too_small(df: pd.DataFrame) -> pd.Series:
    print("[Pattern] pattern_compress_ratio_too_small ...")
    if "compress_ratio" not in df.columns:
        return pd.Series(False, index=df.index)
    # 不能低于 0.2 -> 命中条件是 < 0.2
    return df["compress_ratio"].astype(float) < 0.2
...
PATTERN_FUNCS = [
    ...
    ("control_private_unassigned_ratio_too_large", pattern_control_private_unassigned_ratio_too_large),
    ("compress_ratio_too_small", pattern_compress_ratio_too_small),
...
def main():
    ...
    tasks = [(str(cfg.jsonl), offs, cfg) for offs in chunks]
    # 这里离线脚本本来用 fork 也大概率没事，但为了和在线 server 行为一致、
    # 避免未来从别的服务里 import/main 这个脚本，统一改成 spawn。
    ctx = mp.get_context("spawn")
    samples_list: List[pd.DataFrame] = []
    segments_list: List[pd.DataFrame] = []
    with ctx.Pool(processes=cfg.cpu_procs) as pool:
