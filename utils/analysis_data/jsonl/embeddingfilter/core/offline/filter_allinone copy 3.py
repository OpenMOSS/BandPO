...
def run_hf_embedding(
    ...
    """
    使用纯 HuggingFace Transformers 做 sentence embedding：
    - 默认 mean pooling + L2 normalize
    - 支持 GPU/CPU，支持 dtype 选择
    """
    if segments_df.empty:
        segments_df = segments_df.copy()
        segments_df["emb"] = [[] for _ in range(len(segments_df))]
    ...
    if device.type == "cpu" and torch_dtype in (getattr(torch, "float16", None),
                                                getattr(torch, "bfloat16", None)):
        print("[Stage2-HF][WARN] CPU 上使用 float16/bfloat16 可能不支持，自动退回 float32")
        torch_dtype = torch.float32

    print(f"[Stage2-HF] Loading HF model: {model_name}")
    t0 = time.time()
    ...
    model.to(device)
    model.eval()
    print(f"[Stage2-HF] HF model loaded in {time.time()-t0:.1f}s")

    texts = segments_df["text"].astype(str).tolist()
    n = len(texts)
    print(f"[Stage2-HF] Embedding {n:,} sentences with HF ...")
    ...
    with torch.inference_mode():
        for start in tqdm(range(0, n, bs), desc="[Stage2-HF] Batches"):
            ...
            if hasattr(outputs, "last_hidden_state"):
                ...
                # 大多数 encoder 模型 outputs[0] 即 last_hidden_state
    ...
    segments_df = segments_df.copy()
    segments_df["emb"] = embs
    print(f"[Stage2-HF] HF embedding done in {time.time()-t0:.1f}s")
    return segments_df
...
def main():
    ...
    ap.add_argument(
        ...
        type=int,
        default=64,
        help="HF embedding 的 batch size，默认 64",
    )
    ap.add_argument(
        "--hf-max-length",
        type=int,
