...
# ---------- Stage0: scan offsets ----------
def scan_offsets(jsonl: Path, chunk_lines: int, compute_sha256: bool):
    """
    Sequentially scan JSONL, get (line_no, offset, length) chunks.
    """
    ...
    with jsonl.open("rb", buffering=1024*1024) as f:
        line_no = 0
        processed_bytes = 0
        pbar = tqdm(total=total_bytes, desc="[Scan] JSONL", unit="B", unit_scale=True)
        while True:
            off = f.tell()
            line = f.readline()
            if not line:
    ...
    return chunks, sha256, stat.st_size, int(stat.st_mtime), line_no
...
def main():
    ...
    # filter target 子集
    ap.add_argument("--only-kept", action="store_true", help="只对 kept_flag==1 的样本做 pattern 过滤")
    ap.add_argument("--only-correct", action="store_true", help="只对 cls_id==1 的样本做 pattern 过滤")
    ...
    print(f"[Scan] Scanning offsets in JSONL: {cfg.jsonl}")
    t0 = time.time()
    chunks, sha256, fsize, mtime, n_lines = scan_offsets(cfg.jsonl, cfg.chunk_lines, cfg.compute_sha256)
    print(f"[Scan] Found {n_lines:,} lines, {len(chunks)} chunks; scan took {time.time()-t0:.1f}s")
    ...
    # 如果指定 step_min/step_max，则对 samples 做一次筛选（segments 会自动跟 sample_idx join 时过滤）
    if args.step_min is not None:
        samples_df = samples_df[samples_df["global_step"] >= int(args.step_min)].reset_index(drop=True)
    if args.step_max is not None:
        samples_df = samples_df[samples_df["global_step"] <= int(args.step_max)].reset_index(drop=True)
    # 重新映射 segments 只保留 still valid 的 sample_idx
    valid_sample_set = set(samples_df["sample_idx"].astype(int).tolist())
    if not segments_df.empty:
        segments_df = segments_df[segments_df["sample_idx"].astype(int).isin(valid_sample_set)].reset_index(drop=True)

    total_rows = len(samples_df)
    num_kept = int((samples_df["kept_flag"] == 1).sum())
    num_dropped = int((samples_df["kept_flag"] == 0).sum())
    is_kept = (samples_df["kept_flag"] == 1)
    is_correct = (samples_df["cls_id"] == 1)
    is_wrong = (samples_df["cls_id"] == 0)
    num_kept_correct = int((is_kept & is_correct).sum())
    num_kept_wrong = int((is_kept & is_wrong).sum())
    print(f"[STATS] samples total      = {total_rows:,}")
    ...
    print("[Filter] Building target_mask for pattern filtering ...")
    target_mask = np.ones(len(samples_df), dtype=bool)
    if args.only_kept:
        target_mask &= (samples_df["kept_flag"].to_numpy().astype(int) == 1)
    if args.only_correct:
        target_mask &= (samples_df["cls_id"].to_numpy().astype(int) == 1)

    # ---------- Pattern filtering ----------
    print("[Filter] Running pattern-based filtering on target subset ...")
    t3 = time.time()
    keep_mask, bad_mask, pattern_hits = build_filter_mask_by_patterns(samples_df, target_mask=target_mask)
    ...
    with cfg.jsonl.open("rb", buffering=1024*1024) as fsrc, \
        ...
        # 建 index -> row 的映射，加快查找
        samples_df_index_map = {int(row["sample_idx"]): idx for idx, row in samples_df.iterrows()}
