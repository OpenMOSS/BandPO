...
RUNTIME_ENV="$BandPODir/RLtraining/verl/recipe/dapo/runtime_env.yaml" # RUNTIME_ENV=${RUNTIME_ENV:-"${WORKING_DIR}/verl/trainer/runtime_env.yaml"}

# to modify
project_name="clip_improve_v3"
exp_name="DapoMathDataset_DeepseekR1_1.5B_dapo_official" # no dual-clip, no clip higher
model_path="$BandPODir/data/models/deepseek/R1/DeepSeek-R1-Distill-Qwen-1.5B" # MODEL_PATH=${MODEL_PATH:-"${RAY_DATA_HOME}/models/Qwen2.5-32B"}

max_prompt_length=$((1024 * 2)) # 1024*1时会有部分overlong被filter，DAPO设置了1024 * 2
max_response_length_real=$((1024 * 8)) # 没有overlong_buffer_len的要设置为这个: (1024 * 20)->(1024 * 8)
overlong_buffer_len=$((1024 * 1)) # (1024 * 4)->(1024 * 1)
max_response_length=$((max_response_length_real + overlong_buffer_len))
actor_ppo_max_token_len=$((max_prompt_length + max_response_length))
infer_ppo_max_token_len=$((max_prompt_length + max_response_length))
max_num_batched_tokens=$((max_prompt_length + max_response_length))
train_batch_size=256 # 512->256
ppo_mini_batch_size=64 # 32->64
tensor_model_parallel_size=1 # 4->1
ulysses_sequence_parallel_size=1 # 8->1
gen_prompt_bsz=$((train_batch_size * 1)) # 3->1
max_num_gen_batches=100 # 10->100
rollout_n=16
n_gpus_per_node=8
validate_n=32
test_freq=5
...
kl_delta=0.05
# actor_rollout_ref.actor.fsdp_config.param_offload=True->False
# actor_rollout_ref.actor.fsdp_config.optimizer_offload=True->False

CKPTS_DIR="$BandPODir/data/ckpts/${project_name}/${exp_name}" # CKPTS_DIR=${CKPTS_DIR:-"${RAY_DATA_HOME}/ckpts/${project_name}/${exp_name}"}
...
ray job submit --no-wait --runtime-env="${RUNTIME_ENV}" \
    ...
    +actor_rollout_ref.actor.decaying_clip_methods="cosine" \
    +actor_rollout_ref.actor.decaying_clip_start_value=0.15 \
