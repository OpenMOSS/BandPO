#!/usr/bin/env bash
set -euo pipefail
# set -x # 不在全局开启 xtrace，避免把命令回显刷到终端；在每个评测分组里再 set -x 写入日志
...
date_tag="$(date +%F_%H-%M-%S)" # 统一计算时间戳
# —— 关键改动①：把本脚本剩余所有输出重定向到“顶层脚本日志”，彻底静音终端 ——
top_log_root="${validate_root_dir}/logs/script"
mkdir -p "${top_log_root}"
exec > "${top_log_root}/${date_tag}.log" 2>&1
# （之后任何未显式重定向的 echo/报错都写入上面的顶层日志文件）
...
for model_path in "${model_list[@]}"; do
    ...
    for temperature in "${temp_list[@]}"; do
        ...
        if [[ "$temperature" =~ ^0+(\.0+)?$ ]]; then
            greedy=true
            greedy_note="贪婪解码（do_sample=False，所有数据集 n=1）"
        else
            greedy=false
            greedy_note="采样解码（do_sample=True，按数据集规则设定 n）"
        fi
        # 温度字符串用于命名（把小数点替换为 p）
        ...
        for dataset in "${datasets[@]}"; do
            ...
            # —— 关键改动②：子 shell 分组 + 重定向到各自的 log_file；不再使用 tee，也不往终端打印 ——
            (
                # 只在各自日志里开启命令回显
                set -x
                echo "温度=${temperature} → ${greedy_note}"
